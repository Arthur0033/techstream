...
async function renderPodium(selector, data) {
  ...
  try {
    ...
    drawGroupedBarChart(svg, chartData, dateInfo, width, height, allFlights);
...
function drawGroupedBarChart(svg, data, dateInfo, width, height, allFlights) {
  ...
  const statsPanel = svg.append("g")
    .attr("transform", `translate(${width - margin.right - 165}, ${margin.top + 10})`);
  ...
  const legend = svg.append("g")
    .attr("transform", `translate(${width - margin.right - 165}, ${margin.top + 195})`);
